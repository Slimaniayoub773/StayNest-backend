 ...
 public function register(Request $request)
    ...
    public function googleAuth(Request $request)
    {
        try {
            ...
            if (!$guest) {
                // Create new guest with empty password for Google signups
                $guest = Guest::create([
                    'name' => $payload['name'],
                    'email' => $payload['email'],
                    'password' => '', // Set empty string instead of null
                    'google_id' => $payload['sub'],
                    'is_blocked' => false,
                    'email_verified_at' => now(),
                    'phone' => '000-000-0000', // Temporary placeholder
                    'identification_number' => 'GOOGLE_' . $payload['sub'], // Generate a unique ID
                ]);
            ...
            // If guest exists but has no password (previously signed up with Google)
            if (empty($guest->password)) {
                ...
                return response()->json([
                    ...
                    'token_type' => 'Bearer',
                    'guest' => $guest,
                    'requires_password' => true // Flag indicating password setup is needed
                ]);
            }

            // Regular login for existing users with password
            $token = $guest->createToken('auth_token')->plainTextToken;
